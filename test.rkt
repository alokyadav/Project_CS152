...
;((draw-pixmap window3) "Background.jpg" (make-posn 0 0))
...
(define (draw i j mat)
  ...
  (define a (cond [(= (matrix-ref mat i j) 0) void]
                  ...
                  [(= (matrix-ref mat i j) 2)  (begin
                                                 ...
                                                 ((draw-solid-rectangle window3) (make-posn (* j 30) (* i 30)) 30 30 "gray")
                                                 ((draw-rectangle window3) (make-posn (* j 30) (* i 30)) 30 30 "black"))]
                  ; ((draw-rectangle window) (make-posn (* j 30) (* i 30)) 30 30 "black"))]
...
(define our-tank%
  (class object%
    ...
    (define (kill-x-k x-e)(if(and (>= x-e 290) (<= x-e 340))#t #f))
    (define (kill-y-k y-e)(if(or (>= y-e 620) (<= y-e 630))#t #f))
    (define (all-white-x-k x-e y-e)
      
      (white-x? (+ x-e 10) (+ y-e 30) 630))
    (define (all-white-y-k x-e y-e)
      (if(> x-e 360)(white-y? (+ y-e 10) x-e 360)
         (if(< x-e 300)(white-y? (+ y-e 10) (+ x-e 30) 300))))
